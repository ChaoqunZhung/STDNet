...
class STDNet_Dynamic(nn.Module):
    def __init__(
        self,
        heads,
        ...
        self.head = CenternetHead(heads,in_channel=48)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        fusioned = self.ts_exchange(fusioned)
        output = self.head(fusioned)
        return output

if __name__ == "__main__":
    model = STDNet_Dynamic().cuda()
