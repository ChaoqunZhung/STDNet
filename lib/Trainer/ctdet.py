...
from lib.loss.losses import FocalLoss,MultiClassFocalLoss
...
class CtdetLoss(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.crit = FocalLoss()
        # self.crit = MultiClassFocalLoss()  # torch.nn.MSELoss()
        ...
        # self.crit_reg = nn.SmoothL1Loss(reduction='sum')  # 改用 Smooth L1 Loss 代替 L1 Loss
        # self.opt = opt
        # self.wh_weight = 0.5  # 增大权重，以提高小目标影响力
        # self.hm_weight = 1
        # self.off_weight = 1
        # self.num_stacks = 1
    ...
    def forward(self, outputs, batch):
        ...
        #         # 可以尝试替换nan/inf值
        #         v[torch.isnan(v)] = 0
        #         v[torch.isinf(v)] = 1e6 * torch.sign(v[torch.isinf(v)])
        #         output[k] = v
        output = outputs[0]
        ...
        # pred_boxes = torch.cat([batch['ind'], output['wh']], dim=-1)  # 预测框
        # gt_boxes = torch.cat([batch['ind'], batch['wh']], dim=-1)  # 真实框
        # wh_loss += generalized_box_iou_loss(pred_boxes, gt_boxes, reduction='mean') / self.num_stacks
